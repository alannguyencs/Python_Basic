...
from tkinter import *
import app05_Book_store_backend as backend
...
def get_selected_row(event):
    ...
    #fill tuple in title, author, year, isbn fields
    e1.delete(0, END)
...
def view_command():
    list1.delete(0, END) #clean the list1
    for row in backend.view():
        list1.insert(END, row) #the new row will be put in the end of the listbook
...
def delete_command():
    backend.delete(selected_tuple[0]) #get id
    e1.delete(0, END)
    e2.delete(0, END)
    e3.delete(0, END)
    e4.delete(0, END)
